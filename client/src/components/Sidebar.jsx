      ...
      {/* user list */}
        <div className='flex flex-col'>
          {filteredUsers.map((user,index)=>(
            <div onClick={()=>{setSelectedUser(user); setUnseenMessages(prev => ({...prev,[user._id]:0}))}} key={user._id} className={`relative flex items-center gap-4 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id===user._id && 'bg-[#282142]/50'}`}>
              <img src={(user.profilePic != " ") ? user.profilePic:assets.avatar_icon} alt='Profile' className='w-[35px] aspect-[1/1] rounded-full'/>
