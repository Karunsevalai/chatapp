...
const LoginPage = () => {
  ...
  const [isDataSubmitted,setIsDataSubmtted] = useState(false)
    ...
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
         <h2 className='font-medium text-2xl flex justify-between items-center'>
          {currState}
          {isDataSubmitted && <img onClick={()=>{setIsDataSubmtted(false)}} src={assets.arrow_icon} alt="" className='w-5 cursor-pointer' />}
         </h2>
         ...
         <div className='flex text-sm gap-2 text-gray-500 items-center'>
          <input type="checkbox" />
          <p>Agree to the terms of use & privacy policy.</p>
             ...
             <div className='flex flex-col gap-2'>
              {currState === "Sign up" ? (
                <p className='text-sm text-gray-600 '>Already have an account? <span onClick={()=>{setCurrState("Login");setIsDataSubmtted(false)}} className='font-medium text-violet-500 cursor-pointer'>Login here</span></p>) : (
